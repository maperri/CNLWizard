...
class TestYAMLWriter(unittest.TestCase):
    ...
    def test_py_writer_func(self):
        ...
        self.assertEqual(py_writer.visit_compiled_rule(CompiledRule('test', ['attr attr'])),
                         py_func_str('test', ['attr_1', 'attr_2']))
        self.assertEqual(py_writer.visit_compiled_rule(CompiledRule('test', ['attr ATTR attr'])),
                         py_func_str('test', ['attr_1', 'attr_2']))
        self.assertEqual(py_writer.visit_compiled_rule(CompiledRule('test', ['attr "attr" attr'])),
                         py_func_str('test', ['attr_1', 'attr_2']))
